/* ==============================================================
   FS Workbench — shared document tabs
   File:     js/doc-tabs.js
   Rev:      0.6.0
   Updated:  2026-07-09
...
(function (SH) {
  ...
  SH.docTabs = {
    ...
    report: function (docId, opts) {
      return {
        /* Tabs are kept alive and mount() runs once, so re-read settings
           when the tab is shown again — inclusions may have changed in
           Settings meanwhile. */
        onShow: function () { if (this._host) { this._host.innerHTML = ''; this.mount(this._host); } },
        ...
        mount: function (host) {
          ...
          /* --- table style picker ---
             Reads from settings.tableStyles.list (Table Style tab), writes
             settings.reports[docId].tableStyleId. null = the shipped default. */
          var styleBox = SH.el('div', { style: 'margin-top:16px' });
          styleBox.appendChild(SH.el('h2', { class: 'section' }, 'Table style'));
          styleBox.appendChild(SH.el('p', { class: 'hint', html:
            'Applied to every table in this report. Edit or add styles in ' +
            '<b>Settings → Table Style</b>.' }));
          var styleSel = SH.el('select');
          var currentId = SH.settings.get('reports.' + docId + '.tableStyleId', null);
          styleSel.appendChild(SH.el('option', { value: '' }, 'Default'));
          (SH.settings.get('tableStyles.list', []) || []).forEach(function (s) {
            var o = SH.el('option', { value: s.id }, s.name || '(untitled)');
            if (s.id === currentId) o.selected = true;
            styleSel.appendChild(o);
          });
          if (currentId == null) styleSel.selectedIndex = 0;
          styleSel.addEventListener('change', function () {
            var v = styleSel.value || null;
            SH.settings.set('reports.' + docId + '.tableStyleId', v);
          });
          styleBox.appendChild(SH.el('div', { class: 'field' }, styleSel));
